...
public class CreateRentCommandBackgroundServiceTests : IAsyncLifetime
{
    ...
    [Theory(DisplayName =
        $"{nameof(CreateRentCommandBackgroundServiceTests)}.{nameof(SendCreateRentCommandVerifyEventStatusAndStatusCode)}")]
        ...
        string endpointAction)
    {
        ...
        var response = await _httpClient.PostAsync(endpointAction, httpContent, cancellationTokenSource.Token);
        ...
        do
        {
            ...
            var entityResponse = await _httpClient.GetAsync($"{endpointAction}{commandResponse?.Id.ToString()}",
